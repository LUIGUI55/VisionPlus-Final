...
const videos = [
    ...
    { title: "Bichos: Una aventura en miniatura", bunnyId: "95599932-65df-4412-b2da-d4afacea44b9" },
...
async function mapVideos() {
    console.log("🚀 Starting Batch Video Mapping with Retry Logic...");
    ...
    for (const vid of videos) {
        try {
            ...
            // 2. Map in Backend with Retry
            console.log(`🔗 Mapping to Bunny ID: ${vid.bunnyId}...`);

            let attempts = 0;
            const maxAttempts = 10; // More patience for deployment
            let success = false;
            ...
            while (attempts < maxAttempts && !success) {
                try {
                    ...
                    console.warn(`⚠️ Attempt ${attempts}/${maxAttempts} failed for ${movie.title}. Status: ${status}`);
            ...
            if (!success) console.error(`❌ Failed to map ${movie.title} after ${maxAttempts} attempts.`);
